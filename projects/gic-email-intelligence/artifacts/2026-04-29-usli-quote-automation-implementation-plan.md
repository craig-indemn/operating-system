...
9. **K.9: Add `usli_ref_prefix_to_lob` fixture map if soak shows ambiguous-tiebreak rate > 1% steady-state** — NEW 2026-05-01. The architecture artifact (`2026-05-01-usli-deterministic-lookup-architecture.md` § "Open questions") called for a USLI-ref-prefix → Unisoft-LOB map (MGL→CG, MPL→CP, MSE→MSE, NPP→CG, XSL→CU, ...) for D.2's Tier 2 LOB filter. Decision 2026-05-01 to defer to soak: the LLM classifier already extracts `line_of_business` from the email and D.2 passes it as `--lob` to C.7 directly, so the prefix→LOB lookup table is only valuable as defense-in-depth for multi-LOB-per-insured cases — which are rare in USLI's 97.7%-Retail-Web volume mix. The fixture key was reverted in commit `e7fa472` (`gic-email-intelligence` repo). Add it back ONLY if G.3's ambiguous-tiebreak rate exceeds 1% at steady state, populated from real soak data rather than guesswork. Defer until ≥ 2 weeks post-launch.
