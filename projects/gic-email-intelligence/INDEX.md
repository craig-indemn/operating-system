...
**Migration to Indemn infra (separate workstream — DEVOPS-151) — Phase E + F.1 + H.4 complete 2026-04-27/28.** Read `artifacts/2026-04-27-phase-e-execution-handoff.md` first. Migration branch is 35 commits ahead of `indemn-ai:main` plus 2 follow-on commits on the feature branch. **PR #1 is OPEN** at https://github.com/indemn-ai/gic-email-intelligence/pull/1 (Dhruv has already added a Trivy bump on top — review in progress). Next session pickup is gated on DEVOPS-159 (Dhruv-owned runner registration) and PR #1 merge.
...
- **Outlook add-in is currently broken.** Live deployment at `gic-addin.vercel.app` has a dead Cloudflare tunnel URL (`hrs-cardiff-crop-offering.trycloudflare.com`) baked into its bundle. Decoupled from migration but Phase I.1b rebuild against `https://api.gic.indemn.ai/api` is genuinely required, not optional.
...
| 2026-04-27 | PR #1 on indemn-ai/gic-email-intelligence | https://github.com/indemn-ai/gic-email-intelligence/pull/1 — opens with merge commit reconciling unrelated histories (our 33 + Dhruv's `074a1ec` initial). Force-push blocked by branch protection ruleset 13442013; pivoted to feature-branch + PR. Recommended merge method: "Create a merge commit" to preserve 33-commit granularity. Awaiting Dhruv's review/merge. |
