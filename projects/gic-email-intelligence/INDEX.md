...
**Fresh sessions resuming the rollout: read `artifacts/2026-05-01-usli-prod-rollout-handoff.md` first.** It hydrates the context (project background, 30-min reading list, mental model, hard rules, stakeholders) and points at the deployment checklist as the execution doc. The single-flip moment is `aws ssm put-parameter ... pause-usli-automation false`. Everything else is pre-staged.
...
| 2026-05-01 | [usli-prod-deployment-checklist](artifacts/2026-05-01-usli-prod-deployment-checklist.md) | **Self-contained checklist for the USLI quote automation production rollout.** Captures all identifiers (PR #23, branch, latest SHA 655a950, EC2 instance IDs, Param Store paths, Gmail draft ID, customer mailbox folder), pre-staged work (✅ what's done), hard gates, the H.3–H.6 cutover sequence, rollback procedures, and post-launch backfill scripts (K.8, cutover-window stranded). Paired with the rollout handoff (below) — handoff for context, checklist for execution. |
| 2026-05-01 | [usli-prod-rollout-handoff](artifacts/2026-05-01-usli-prod-rollout-handoff.md) | **Resume-handoff for fresh Claude sessions executing the production rollout.** Models the pattern from `2026-04-29-feature-scoping-handoff.md`: 30-min layered reading list, mental model, pre-staged checklist, stakeholders, hard rules, git layout, conventions, "your first response to Craig" framing. Points at `usli-prod-deployment-checklist` as the execution doc. **Fresh sessions resuming the rollout start here.** |
