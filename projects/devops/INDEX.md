...
## Status
Fourteenth session (2026-04-27 evening): **Jonathan's Observatory bug → fixed end-to-end + AI-370 CLI follow-up shipped.** Detailed write-up in observatory project: [observatory/artifacts/2026-04-27-voice-ingestion-and-vertex-swap.md](../observatory/artifacts/2026-04-27-voice-ingestion-and-vertex-swap.md). Two-bug day: (1) **Voice ingestion never deployed to prod** — PR #19 (Mar 6) added Langfuse trace sync but the prod lambda + Step Functions state machine never got promoted from dev. Diagnosed, deployed lambda + state machine update + 49-day backfill. 120 voice obs convos restored across 12 agents. DEVOPS-148 + 4 children all in Acceptance. (2) **Anthropic credit balance hit zero in April** — classifier was silently writing default fallback values, polluting ~950 chat conversations. Switched to Gemini 2.5 Flash on Vertex AI (mirrors gic-email-intelligence pattern, no application code change — env config + SA file mount only). Deleted + re-ingested 943 April fallback records, 616 went from "unknown" to real success/failure/partial outcomes. Source PRs `indemn-ai/Indemn-observatory#72` (Vertex env) + `#73` (lambda filename normalization), both with Rudra+Dhruv. COP-507 + 2 children in Acceptance. (3) **Adjacent AI-370 from Jonathan's saved item** — CLI couldn't write `transfer_call.handoffNumber` field (no first-class flag). Created Linear issue, shipped fix as `indemn-ai/indemn-cli#14` stacked on PR #13, replied to Jonathan in AI-369 thread. (4) Saved-for-Later queue cleared 2 items (Jonathan Observatory bug + Jonathan transfer_call followup); 3 remaining are Cam's Demo #1 script edits, Craig's own self-bookmark, and the still-in-flight AI-369 saved item.

**Next session — primary (tomorrow morning)**: 1) Verify the 01:00 UTC scheduled SFN run on 2026-04-28 fired both `sync_traces` AND `sync_langfuse_traces` branches successfully (`aws stepfunctions list-executions ...`). 2) Check PRs Indemn-observatory#72/#73 + indemn-cli#13/#14 for review activity. If approved + merged → move associated Linear issues from Acceptance to Done. 3) If Jonathan replied in either thread, address. See [observatory INDEX](../observatory/INDEX.md) for the structured pickup checklist.

**Next session — secondary**: still passively unblocking — when indemn-cli PR #13 lands → `git push origin main:prod` → publish 1.5.0 → close out remaining AI-369 saved item. PR #14 rebases onto main once #13 merges. form-extractor PR #17 still awaiting Dolly re-review.

**Cleanup carried over**: jarvis-cli-mcp project INDEX still says version 1.1.1; should be updated to reflect 1.4.0 published + 1.5.0 in flight.
