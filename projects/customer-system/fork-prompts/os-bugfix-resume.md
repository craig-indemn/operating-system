...
- 🟡 **#2 — Meeting create HTTP 500** — **UNBLOCKED.** `indemn-api` is now redeployed with the new error handler. Next session (or main session if it gets to this) retries `POST /api/meetings/` with the Alliance trace payload; the actual exception will surface in the 500 body. Likely same root as Bug #25 (Company) and Bug #26 (Deal).
