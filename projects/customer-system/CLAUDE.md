...
### Session 14 (2026-04-30) — TD-1 substantial execution; Drive end-to-end; Slack adapter built; voice v1 wrong shape; Bug #38/#41/#42 closeouts

The biggest single execution session on the customer-system project. TD-1 ~85% closed. Three of four scheduled fetcher actors running autonomously. Drive + Slack adapters built end-to-end. Voice harness v1 built but architecturally wrong (flagged for canonical rebuild next session). Bug #38, #41, #42 resolved.
...
- ReviewItem entity + Reviewer role created. Smoke-tested end-to-end (created → watch fired → message routed to reviewer queue → resolution flow). Craig assigned to reviewer role.
- **Email-Fetcher activated end-to-end** (id `69f2bf30942e5629f07a8313`, cron `*/5 * * * *`, gemini-3-flash-preview/global). Trace `019ddf95-d579-7390-9483-beece987389f` 18:10-18:15Z verified: 7 LLM turns all `finish=STOP`, agent fetched 326 new emails. Autonomous, currently running.
- **Meeting-Fetcher activated** (id `69f39ec6c0b340cf765a38d6`, cron `*/15 * * * *`). 30-day backfill returned fetched=396 created=2 skipped=394 errors=0.
- **Drive adapter NEW kernel build** (commit `c87376d`): `fetch_documents()` on GoogleWorkspaceAdapter (per-user DWD impersonation, paginate by pageToken, dedup by drive_file_id, format Document-shaped dicts with `external_ref = drive_file_id`). 6 unit tests in `TestFetchDocumentsContract` GREEN. Document `external_ref` field added (sparse+unique). fetch_new capability activated. Drive-Fetcher actor (`69f3abbe268936150b46a0fa`, hourly cron) activated. 30-day backfill: fetched=1161 created=493 skipped=668 errors=0.
- **Slack adapter NEW kernel build** (commit `c87376d`): `kernel/integration/adapters/slack.py` — Slack Web API via httpx, conversations.list/history with cursor pagination, strict params, rate-limit + auth typing, SlackMessage-shaped dicts with composite `external_ref`. 9 unit tests in `TestSlackAdapterContract` GREEN. Self-registered as `register_adapter("messaging", "slack", ...)`. SlackMessage entity created (16 fields, state machine). fetch_new capability activated. Slack Integration entity created + transitioned to active (id `69f3bb5097300b115e7236dd`, secret_ref `indemn/dev/integrations/slack-oauth`). Bot token in AWS Secrets. **Live fetch BLOCKED — Bug #45 logged.**
...
- **voice-deepagents v1 built** (commit `62f47f9`) — uses `livekit.agents.Agent` with single custom `execute` tool. **Architecturally wrong** per `docs/architecture/realtime.md` + `associates.md` which require deepagents framework + Interaction/Attention lifecycle + harness_common modules + three-layer LLM config + DeepagentsLLM adapter wrapping the agent for LiveKit's AgentSession. **v1 flagged for DELETE + REBUILD next session.** Voice runtime entity (id `69f3b7fc97300b115e7236a0`) + service token in AWS Secrets ready for v2.

**Bug #42 reframe + resolution.** Original framing (agent reads skill but doesn't act) was wrong. **Actual root cause** (verified 100% repro across 3 LangSmith traces): Gemini 2.5 Flash returns `MALFORMED_FUNCTION_CALL` on the deepagents `write_todos` schema — emits Pythonic compositional-tool syntax (`print(default_api.write_todos(todos=[default_api.WriteTodosTodos(...)]))`) instead of valid JSON. Resolution: switched to `gemini-3-flash-preview/global`. Both runtime defaults flipped (async-deepagents-dev + chat-deepagents-dev). IE picked up the new model on its next run.

**LangSmith query gotcha discovered:** `order: "DESC"` (uppercase) returns HTTP 422 silently; must use `"desc"` (lowercase). The fork's earlier "0 traces visible" symptom was this query bug — traces had been arriving the entire time post-LangSmith-wiring. CLAUDE.md § 8 LangSmith debugging recipe should add a one-liner about lowercase `order`.

**Parallel fork session early in Session 14** — closed mid-session with 3 indemn-os commits. Bug #38 (queue dispatch jam: uncaught WorkflowAlreadyStartedError + stale message retries + suspended-actor watch fan-out) resolved with three-coupled-causes / one-PR fix (`1026d78` + `c36969b`, 16 unit tests). Bug #37 follow-on (bulk-delete poison on malformed entities) resolved by propagating `skip_invalid=True` to bulk activities + DELETE-cleanup pass (bundled in `1026d78`). Bug #41 (harness `_scheduled` entity-load CLI failure) resolved with `_load_message_context` extraction branching on `entity_type.startswith("_")` (`96684d5`, 6 unit tests). All 1015+ pending email_classifier messages parked cleanly via the new `parked` status + WorkflowAlreadyStartedError catch.

**Load-bearing turns:**

1. **TD-1 was bigger than the roadmap implied for Drive.** "Drive adapter exists; needs systematic ingestion wired" was wrong — adapter had only `_drive_service` helper, no `fetch_documents`. Built the actual adapter end-to-end (Bug #43 closed in-session) instead of deferring. Sets the precedent: when the roadmap understates scope, build the missing piece in-session if compute permits, don't fork to a future session.

2. **Bug #42 surfaced the LangSmith query gotcha.** Without the LangSmith trace inspection, Bug #42 would have been mistakenly attributed to skill-content / agent-compliance — leading to wasted iteration on the prompt. With the trace (once the `order: desc` bug was fixed), the actual root cause was visible in 5 minutes. Verifies the Session 10 lesson: "every 'the agent did the wrong thing' should flip to 'let me look at the trace' before 'let me iterate the skill.'"

3. **The canonical voice harness pattern is in `docs/architecture/realtime.md`.** Mid-session Craig pushed back: "I would read files to understand first." Reading realtime.md + associates.md revealed the canonical pattern: voice harness uses deepagents (same agent code as chat-deepagents) + Interaction/Attention lifecycle + harness_common + three-layer LLM config + LiveKit only as audio I/O via DeepagentsLLM adapter. v1 (built before reading the docs) violated this pattern fundamentally. **Lesson reinforced (matches `feedback_understand_before_fix.md`):** read the architecture docs BEFORE building substantial new infrastructure. The OS docs encode the canonical pattern; deviating creates two stacks instead of one.

4. **Per Craig's correction "we are implementing EVERYTHING to completion"** — there is no "fork session" hedge or "tier framing" (D-min/D-mid/D-full). The implementer is the implementer. Defer-language ("multi-session work," "another fork session") often masks unwillingness to engage with scope; the right response to substantial scope is to build, not to defer.

5. **Three runtime layer flips matter.** `async-deepagents-dev` + `chat-deepagents-dev` runtimes both flipped to `gemini-3-flash-preview/global`. Voice runtime created with same default. Bug #42 fix cascades to all 23+ associates without per-actor llm_config overrides. Per-actor overrides are escape hatches, not the default mechanism.

**Handoff to Session 15:** Use `PROMPT.md` as kickoff. Two priorities: (1) Bug #45 — debug Slack `resolve_integration()` lookup; resolve, then 90-day backfill + Slack-Fetcher actor build + activation closes TD-1 sub-piece 4. (2) Voice-deepagents canonical rebuild — DELETE v1 files; REBUILD mirroring chat-deepagents structure (agent.py from chat-deepagents/agent.py, session.py with Interaction + Attention + heartbeat, llm_adapter.py = DeepagentsLLM(livekit.agents.llm.LLM), main.py with WorkerOptions). Voice runtime entity + service token already in place. After both: TD-1 done-test verification + close. Then TD-2 cascade activation.

**Material state at close:** TD-1 ~85% complete. Three of four fetcher actors active and autonomous. Pipeline EC/TS still suspended; IE active and now on gemini-3-flash-preview. ReviewItem + Reviewer wired. log-touchpoint skill assigned to OS Assistant. Slack Integration in place but live fetch blocked by Bug #45. Voice runtime + token ready for v2 rebuild. Bug list converged on Bug #44 (voice rebuild) + Bug #45 (Slack dispatch) + carry-forward Bug #40 (deterministic scheduled-actor execution path — long-term architectural).

### Session 15 (2026-05-01) — TD-1 fully closed; voice harness v2 deployed; chat + voice migrated to CLI-only skill loading; 7 OS bugs resolved

The session that closed TD-1 end-to-end. Massive validation + cleanup + harness architecture cleanup.

**TD-1 closure (3 verifications):**
- **Slack-Fetcher activated.** Bug #45 resolved as DUAL fix: (a) Slack Integration `access: null` workaround via data patch; (b) `register_adapter("messaging", "slack", SlackAdapter)` had wrong arg order (passed system_type+provider instead of provider+version) → registered at key `messaging:slack` instead of `slack:v1` → `AdapterNotFoundError` on every fetch_new. One-line fix: `register_adapter("slack", "v1", SlackAdapter)` (`eea170d`). 90-day Slack backfill drained 860 messages spanning 2023-11-17 → 2026-05-01 across 4 channels (conversation-design 435, voice 217, customer-implementation 200, test-reports 8). Slack Fetcher actor `69f4a473a0cbb2f5d2d0386f` cron `*/5 * * * *` active and autonomous; first tick verified via LangSmith trace `019de3c9-ab78-7383-b273-5c0d051d9431`.
- **Voice harness v2 canonical rebuild + Railway deploy.** v1 (`62f47f9`) deleted; v2 mirrors chat-deepagents structure: `agent.py` (deepagents `create_deep_agent` + voice DEFAULT_PROMPT), `session.py` (VoiceSession with Interaction + Attention + 30s heartbeat + `indemn events stream` subprocess), `llm_adapter.py` (`DeepagentsLLM(LLM)` adapter wrapping the deepagents agent for LiveKit's AgentSession), `main.py` (LiveKit `WorkerOptions(entrypoint_fnc, prewarm_fnc)`). 17 unit tests. Indemn-os main commits `6671dea` (v2) + 7 follow-ups for bootstrap, env-var fallbacks, agent_name dispatch, sys.executable indemn resolution, LangSmith metadata, Dockerfile CMD, drop turn_detector. Deployed to NEW Railway service `indemn-runtime-voice` (id `df2349d6`). Multi-turn voice round-trip created Touchpoint `69f4ed4f03e56394d808bc88` (Branch + Dan Spiegel + summary "discussed renewal pricing and timeline").
- **Chat-side log-touchpoint verified.** Touchpoint `69f4f2ca03e56394d808bd6d` created via deployed `indemn-runtime-chat` after migration to CLI-only skill loading (Bug #47 fix required first — chat service token in Railway env was stale).

**Architectural cleanup:**
- **chat + voice harnesses migrated to CLI-only skill loading.** Mirrors async-deepagents `7281b83`. New `build_system_prompt(associate)` helper composes per-associate system prompts with `execute('indemn skill get <ref>')` directives. Drops the deepagents filesystem-skills layer entirely. Eliminates the Bug #35 class for both harnesses. (`7197f08`)
- **`harness_common.cli` resolves indemn via `sys.executable`.** Eliminates the `/opt/homebrew/bin/indemn` (Node.js CLI from `@indemn/cli` npm package) collision that was silently breaking the harness in spawn-mode subprocesses. (`33d12f7`)
- **Voice DeepagentsLLM passes RunnableConfig with metadata + tags + run_name** (`27e9304`). Voice traces appear in `indemn-os-associates` LangSmith project queryable per CLAUDE.md § 8.

**OS bugs resolved (7):**
1. **Bug #45a** (data patch): Slack Integration `access.roles` populated.
2. **Bug #45b** (`eea170d`): Slack adapter register_adapter args fixed.
3. **Bug #45c** (`5036bc1`): kernel resolver Step 3 query uses `$or` to tolerate null/missing `access`. 5 unit tests.
4. **Bug #46** (`20c074c`): kernel `fetch_new` watermark fallback chain (`date`→`posted_at`→`created_date`). 8 unit tests. Verified live: Document fetch-new immediately surfaced 5 NEW Drive files post-fix.
5. **Bug #47** (Railway env update + redeploy): chat-deepagents stale service token resolved.
6. **Bug #12** (AWS Secrets update): `mongodb-uri` corrected to working host.
7. **Bug #13 followup** (`bee1a7e`): Voice harness rows added to development.md deploy table.
8. **Bug #39** (`00b7407`): `_route_slug_for` honors `_collection_name` operator override + `Settings.name` for kernel entities. 5 unit tests.
9. **Bug #44** closed: voice harness fully canonical + deployed end-to-end.

**Load-bearing turns:**

1. **Bug #45 was a dual fix that required both layers.** The data patch alone wasn't enough; the adapter registration call was also wrong. Fixing only one would leave the system silently broken. Lesson: when multiple integrations of the same `system_type` work but a new one doesn't, suspect BOTH the data shape (per-integration config like access.roles) AND the code path (per-integration registration). Don't stop at the first plausible cause.
2. **The deepagents filesystem-skills layer was wrong shape for ALL three harnesses, not just async.** Async-deepagents migrated 2026-04-29 (Session 12, commit `7281b83`); chat + voice followed Session 15 (Session 12 plan tracked it). Three concrete benefits: eliminates Bug #35 class entirely (no path resolution against backend root_dir, no YAML escaping, no SKILL.md format), symmetric with how the agent loads entity skills + everything else in the OS, gives operators a system_prompt directive they can directly read.
3. **Subprocess env propagation in spawn-mode multi-process workers (LiveKit Agents) is unreliable for PATH.** `harness_common.cli` previously called `subprocess.run(["indemn", ...])` relying on PATH. Different binary (`/opt/homebrew/bin/indemn` is Node.js CLI from `@indemn/cli` npm package — only has `init`) gets picked up sometimes. Fix: resolve absolute path via `sys.executable`'s parent dir at module load. Generalizable lesson: any harness shell-out should pin to absolute paths, not rely on PATH inheritance.
4. **Kernel `fetch_new` watermark assumed `entity.date`** — silently broken for SlackMessage (`posted_at`) + Document (`created_date`). Symptom invisible to logs (no error, just `created=0`). Surfaced only by running the actual flow on real backfilled data and noticing the drain pattern doesn't converge. Trace-as-build success.
5. **Resolver `access: null` → unreachable** is the wrong default for org integrations. Operator intent for null is "no role gate, any actor in this org can use it" — not "no access." Bug #45c fix uses $or to make both work; better error message uses `Integration.find().count()` to hint at status/access mismatch when lookup fails on existing-but-non-matching integrations.
6. **Voice harness ports 1:1 from chat structure once you have DeepagentsLLM.** The DeepagentsLLM adapter (translates LiveKit `ChatContext` ↔ deepagents `messages` + invokes `agent.ainvoke()` + emits `ChatChunk`s) is the bridge that lets all three harnesses (async, chat, voice) share the same agent + skill loading + Interaction/Attention lifecycle. Confirms the canonical pattern: agent code is harness-agnostic; only I/O transport differs.
7. **End-of-session bug-cleanup discipline pays off.** 7 OS bugs resolved + 14 indemn-os commits in one session. Test count grew 437 → 456 (kernel suite). The fork-and-coordinate pattern from prior sessions wasn't needed because the work fit one main thread.

**Material state at close:** TD-1 fully closed end-to-end. All 4 fetcher actors active and autonomous. Voice + chat round-trips both create real Touchpoints. Voice OS Assistant created and bound to Railway voice runtime. 7 OS bugs resolved. Bug list converged on Bug #40 (deterministic scheduled-actor execution path — deferred to Session 16 for deep design per Craig) + Bug #19 follow-on (process improvement, no code action). 14 indemn-os main commits this session.

**Handoff to Session 16:** Use `PROMPT.md` as kickoff. Objective: deep design + implementation of Bug #40. Pick (A) `cron_runner` actor mode or (B) new `ScheduledActorWorkflow` with rationale, execute, deploy, verify all 4 fetchers continue working post-migration. Then begin TD-2 (cascade activation: build MeetingClassifier, SlackClassifier, Proposal-Hydrator, Company-Enricher; activate progressively bottom-up; systematic historical replay across the ~930 emails + 67 meetings + 860 SlackMessages).

### Session 16 (2026-05-01 → 2026-05-04) — Bug #40 closed via `cron_runner` actor mode (Option A); Bug #48 (CLI URL slug) + Bug #49 (cron_runner heartbeat + OTEL span) surfaced and closed in same session; 3-day soak verifies 1863 cron_runner completions / 0 LangSmith traces / ~1000 LLM calls/day eliminated

The session that closed Bug #40 end-to-end and unblocked TD-2. Spans 2026-05-01 (initial implementation + flip + Bug #48 surfacing) and 2026-05-04 (3-day soak verification + Bug #49 root-cause + heartbeat fix + EOS). Three bugs closed (#40, #48, #49); one bug re-fixed (#12); one observation logged (Bug #45-family Slack Integration auto-error one-off).

**Bug #40 — cron_runner actor mode (the headline work).**
- **Option A chosen over Option B** with rationale: extends the existing `Actor.mode` Literal (`deterministic | reasoning | hybrid | cron_runner`) rather than introducing a parallel kernel-side workflow type. Uses Bug #41's existing harness foothold (`_load_message_context` already branches on `_*` synthetic types). Routes through unchanged dispatch path. ~5-7 files touched vs ~2x for Option B with no observable runtime difference. Per CLAUDE.md "no abstraction for abstraction's sake."
- **Implementation:** `kernel_entities/actor.py` adds `cron_runner` to mode Literal. `harnesses/async-deepagents/cron_runner.py` NEW (~200 LOC): `parse_command_from_skill` extracts argv from `## Command` section + bash fence + indemn-only allowlist + single-command-only constraint. `run_cron_skill` validates trigger is synthetic `_*`, loads first skill via existing CLI, parses, exec via `harness_common.cli.indemn()` (env-var propagation INDEMN_EFFECTIVE_ACTOR_ID + INDEMN_CAUSATION_MESSAGE_ID intact), inspects JSON `errors` field (non-empty → fail), marks queue complete/fail. `harnesses/async-deepagents/main.py::process_with_associate` branches on `actor.mode == "cron_runner"` BEFORE building the agent — skips deepagents entirely.
- **22 unit tests in v1** (parser × 11, executor happy + 7 failure modes × 8, sync-helper pin, mode-Literal pin via Pydantic field annotation introspection — Beanie's `init_beanie` requirement made instantiating Actor in tests difficult so I pin via `model_fields["mode"].annotation` introspection instead).
...
- **Live verification 2026-05-01 19:59:58Z (flip):** all 4 fetcher actors flipped to `mode=cron_runner` via `PUT /api/actors/<id>`. cron_runner exec + success log lines visible in runtime logs for all 4. **0 LangSmith deepagents traces post-flip across the 3-day soak window.**

**Bug #48 — CLI URL slug regression** (NEW; surfaced from Session 15's Bug #39 server-side fix being incomplete).
- Slack-Fetcher cron_runner exec hit 404 on `indemn slackmessage fetch-new`. Server-side route is `/api/slack_messages/` (Bug #39 fix); CLI client was hitting `/api/slackmessages/` (naive plural).
- **Two-part fix:** (1) `kernel/api/meta.py::get_entity_metadata` (list endpoint) added `collection` field via `_route_slug_for(name, cls)`. Detail endpoint had `collection` but used the broken `cls.Settings.name if hasattr(cls, "Settings") else entity_name.lower() + "s"` fallback (which returned naive plural for every domain entity since they don't have Beanie Settings — collection_name lives on `_collection_name`). Replaced with `_route_slug_for(entity_name, cls)`. (2) `indemn_os/src/indemn_os/main.py` split `cli_name = name.lower()` (singular Typer subcommand — operators still type `indemn slackmessage list`) from `slug = meta.get("collection") or (cli_name + "s")` (URL collection slug). All `f"/api/{slug}s/..."` templates → `f"/api/{slug}/..."`. `register_bulk_commands(name, entity_app, url_slug=slug)` plumbs the URL slug to bulk_commands.
- **9 unit tests** in `tests/unit/test_meta_collection_field.py` × 4 + `tests/unit/test_cli_url_slug_resolution.py` × 5. Source-level pins via `inspect.getsource` to guard against the one-line regression that broke this.
- **Deployment:** indemn-api + indemn-runtime-async redeployed. Local CLI also re-installed in bot-service venv (`/Users/home/Repositories/bot-service/.venv/bin/pip install -e /Users/home/Repositories/indemn-os/indemn_os/`).

**Bug #49 — cron_runner heartbeat + OTEL span** (NEW; surfaced from 3-day-soak dead_letter investigation on 2026-05-04).
- 11 dead_letter messages over the 3-day soak (5 Email + 6 Slack + 0 Meeting/Drive — Slack-heavy). Initially framed as "Bug #38 orphan cleanup." Direct investigation via `temporal workflow describe msg-69f81d4a1f2c3ee82ecb65bf` returned `Status: FAILED, Cause: activity Heartbeat timeout`, RunTime 8m15s — confirmed root cause is heartbeat timeout, not infrastructure restart.
- **Root cause:** `process_with_associate` activity has `heartbeat_timeout=timedelta(seconds=90)`. The agent path runs `_heartbeat_loop` every 30s during `agent.ainvoke()`. The cron_runner branch shipped in Bug #40 v1 had NO heartbeat — `run_cron_skill` (sync) blocked on `subprocess.run`. Fetches under 90s succeeded; fetches over 90s (Slack with rate-limited channels) hit heartbeat_timeout, activity cancelled, both retries hit identical timing, workflow ended FAILED, message orphaned at status=processing → visibility timeout → next dispatch sweep → WorkflowAlreadyStartedError → Bug #38 cleanup → dead_letter. **The Slack > Email > 0 split is the predicted pattern.**
- **Fix:** in main.py's cron_runner branch, mirror the agent path: `activity.heartbeat("starting_cron_runner")` immediately, then `asyncio.create_task(_cron_heartbeat_loop)` (sleeps 30s + heartbeats), wrap sync `run_cron_skill` in `await asyncio.to_thread(...)` so heartbeat runs concurrently. Cancel + await heartbeat task in finally. Activity-level concern stays in the calling activity — keeps `run_cron_skill` sync + simple.
- **OTEL span added in same fix:** `cron_runner.run` span in `cron_runner.py` via `opentelemetry.trace.get_tracer(__name__)` with attributes `associate.id`, `associate.name`, `message.id`, `entity.type`, `argv`, `tool`, `result.fetched|created|skipped_duplicates`, `result.errors_count`, `outcome`. Span lives under the parent activity span (TracingInterceptor). Per vision §2 item 7 — OTEL for system-level observability, LangSmith for AI-agent observability. cron_runner is system-level so OTEL is the canonically-correct surface; LangSmith stays at 0 traces for cron_runner runs and that's the **correct** state, not a gap.
- **3 new tests** (heartbeat shape pin × 1 via source inspection; OTEL span × 2 using `opentelemetry.sdk.trace.export.in_memory_span_exporter`).
- **Deployment:** indemn-runtime-async redeployed.

**Bug #12 re-fix.** AWS Secret `indemn/dev/shared/mongodb-uri` was reverted between Session 15 and Session 16 (host back to broken `dev-indemn-pl-0`). Re-applied the same fix (pull working URI from chat-deepagents Railway env, `aws secretsmanager update-secret`). No code change. Open question: what reverted it?

**Slack Integration recovery.** Found at `status=error` (auto-transitioned 2026-05-01 18:53:00Z, before Session 16 started). Walked `error → configured → connected → active`. Stayed active through 769+ Slack-Fetcher cron_runner completions over 3 days — one-off, not recurring. Root cause not pinned (couldn't find the kernel handler that transitioned it; effective_actor_id was null in the change record).

**Cumulative cron_runner soak (3 days, pre-Bug-#49-fix metrics):** 1863 completions across 4 fetchers. 11 dead_letter (all root-caused to Bug #49). 0 LangSmith deepagents traces. **~1000 LLM calls/day eliminated as designed.** Bug #49 fix expected to drop dead_letter rate to near-zero over next 24-48h.

**Load-bearing turns:**

1. **Choosing Option A (cron_runner mode) over Option B (ScheduledActorWorkflow peer)** — extends the existing Actor.mode primitive rather than introducing a parallel kernel workflow type. Half the work, no runtime difference, aligns with "no abstraction for abstraction's sake." When two designs reach the same end-state, the one that extends an existing primitive wins.
2. **Bug #48 was the second half of Bug #39.** Server-side route fix (Session 15) without the matching CLI client fix (Session 16) created a half-deployed state that 404'd on every entity where `collection_name != name.lower() + "s"` (currently just SlackMessage). Lesson: when a fix changes URL semantics, audit BOTH the server and every client of the API — including the CLI which is itself a client.
3. **Bug #49 was a real bug masked by the wrong frame.** "11 dead_letters via Bug #38 orphan cleanup" sounds like infrastructure noise; it's not. Direct Temporal investigation took 5 minutes and produced the actual root cause. **Lesson reinforced (matches Session 10's "every 'the agent did the wrong thing' should flip to 'let me look at the trace' before iterating the skill"):** every "0.6% drop rate, probably runtime restarts" should flip to "let me describe the workflow" before declaring it acceptable.
4. **Per Craig's directive — bugs should be bugs, not deferred.** I initially proposed deferring the dead_letter investigation to a future session ("not blocking, just noise"). Craig pushed back: we have context, we should resolve. Right call — the investigation took ~10 minutes and surfaced a real heartbeat-timing bug in Bug #40 v1's shipping. **Lesson:** "we'll look at it later" is a tell that something might be wrong with my framing. When pushed back, investigate.
5. **OTEL is the canonical observability surface for system-level work; LangSmith is for AI-agent observability.** I overcorrected when Craig asked "why are we closing with 0 LangSmith traces?" — proposed adding LangSmith trace per cron_runner exec. Walking back: cron_runner is system-level; the right surface is OTEL spans (already wired via TracingInterceptor); the canonical answer per vision §2 item 7 is LangSmith stays at 0 for non-AI work and that's correct. Adding the `cron_runner.run` OTEL span gives Grafana the per-associate filter that matched the LangSmith muscle-memory recipe in CLAUDE.md § 8 — best of both worlds without conflating the surfaces.
6. **Bug #12 secret regression suggests an undocumented automated process reverts the secret.** Two sessions in a row, the same fix has been needed. Worth a pre-session-hook validation script in the future.

**Material state at close:** All 4 fetcher actors at `mode=cron_runner` with `cron_runner.run` OTEL spans + heartbeat-loop coverage. Pipeline EC/TS still suspended; IE active (unchanged from Session 15). Slack Integration `active`. AWS Secret mongodb-uri working. **3-day soak metrics: 1863 cron_runner completions, 0 LangSmith deepagents traces, dead_letter rate dropping near-zero over next 24-48h post-Bug-#49 fix.** Pricing Framework Session 3 ran in parallel and reached Phase D in-progress state — different worktree, no cross-impact.

**Handoff to Session 17:** Use `PROMPT.md` as kickoff. Objective: **TD-2 cascade activation begins.** 4 NEW associates to build (MeetingClassifier, SlackClassifier, Proposal-Hydrator, Company-Enricher) per `roadmap.md § TD-2`; update EC v9 → v10 (signature parsing + ReviewItem-on-ambiguity); update TS v6 → v7 (Deal-creation + atomic Proposal-at-DISCOVERY + multi-Deal ambiguity → ReviewItem); IE full-cascade verification through TS-created Touchpoints + Option B source pointers + Deal/Proposal existence; activate progressively bottom-up; done-test = systematic historical replay across ~930 emails + 67 meetings + 860 SlackMessages. Trace-as-build-method per § 2 — for each new associate: pick a real scenario (Armadillo's Apr 28 discovery meeting for MC; Apr 7-8 Retention Associate Slack thread for SC; Armadillo's processed Touchpoints for PH; Armadillo's bare Company for CE), trace manually first via CLI, write skill from what worked, activate after the trace produces correct state on real data. Multi-session work; close cleanly per session. **Plus 24-48h Bug #49 verification** — confirm dead_letter rate dropped to near-zero on cron_runner runs after the heartbeat fix deployed today.

### Session 17 (2026-05-04) — TD-1 verified done end-to-end; Bug #50 (queue visibility-extend + attempt_count cap) + fetch_new chunk-cap with oldest-first sort + Bug #12 reframe + 7 zombie polling loops killed; pre-TD-2 OS hardening sprint planned for next session

The session that took TD-1 from "marked complete in roadmap.md" to "verified done end-to-end on production state" — and shipped the Tier-zero fixes that made it true. Started as the kickoff TD-2 cascade-activation session per Session 16's handoff, but pre-flight Bug #49 verification surfaced a chronic Email Fetcher subprocess slowness + multi-pod completion race that had to be resolved before the cascade could land cleanly. Three coupled bugs fell out of the verification; all three fixed in-session.

**Bug #50 (queue visibility extend + max_attempts cap) closed.** Two coupled defects:
- **(50a)** Bug #49 (Session 16) added a Temporal activity heartbeat in cron_runner — kept the activity alive past 90s `heartbeat_timeout`. But Mongo's `message_queue.visibility_timeout` (5 min, set on every claim in `kernel/message/mongodb_bus.py`) was independent — nothing extended it while the runtime was still working. Slow Email/Slack subprocesses raced the queue's recovery sweep: pod A still working, queue recovers at 5 min, pod B claims it (multi-pod is by design), pod A's later `complete` hits 404 "Message not found." Subprocesses succeed but bookkeeping fails, watermark never advances, backlog grows.
- **(50b)** `kernel/queue_processor.py::check_visibility_timeouts` unconditionally recovered every timed-out `processing` message back to `pending` with no max_attempts check. The bus's `claim` path increments `attempt_count` on every claim, but nothing capped it — so a stuck message could attempt 7+ times indefinitely (observed: stuck email_fetcher message `69f89bec1f2c3ee82ecb66c4` at attempt_count=7 / max_attempts=3). The explicit `bus.fail` path enforced max_attempts; this implicit recovery path was the gap.

Fix landed in indemn-os main `18ab3b9` (merged via `7c3a54c`): new `POST /api/message_queues/{id}/extend-visibility` endpoint in `kernel/api/queue_routes.py` (idempotent on terminal status; refuses pending); new `indemn queue extend-visibility <id>` CLI in `indemn_os/src/indemn_os/queue_commands.py` mirroring complete/fail verbs; cron heartbeat loop in `harnesses/async-deepagents/main.py` now calls `await asyncio.to_thread(indemn, "queue", "extend-visibility", message_id)` alongside `activity.heartbeat()` (same 30s cadence; CLIError caught + logged); `check_visibility_timeouts` split into two `update_many` calls — first dead-letters messages where `attempt_count >= max_attempts` via `$expr: {$gte: ["$attempt_count", "$max_attempts"]}`, then recovers the rest. 12 new unit tests + 1 harness shape pin. Verified live: vis_gap > 5min observed on multiple email_fetcher messages (10.9min, 8.8min, 7.8min); queue_processor logs `Dead-lettered N messages over max_attempts (Bug #50)`. Email Fetcher first post-fix completion at **2026-05-04T21:33:49Z** — first email completion since 14:26 UTC that day (7+ hours of stuckness ended).

**`fetch_new` chunk-cap + oldest-first sort shipped (Bug #50 follow-on).** Bug #50 fix proved Email Fetcher *could* now complete via heartbeat extension, but each subprocess was still chronically slow due to per-entity sequential `save_tracked()` loop in `kernel/capability/fetch_new.py` — 11-mailbox fan-out × ~150-300ms per save. Verified there is **NO LLM call** in the email fetch path (cron_runner mode is genuinely deterministic; LangSmith traces stay at 0). Bridging fix in indemn-os main `06d2bbd` (merged via `a09c67b`): `params["limit"]` caps saves per call. **Critical correctness invariant:** when `limit` is set, the cap must apply to OLDEST-first slice — otherwise APIs that return newest-first (Gmail's default) would advance the watermark past unsaved older items, leaving them stranded forever. Implementation sorts new items ascending by watermark field (date / posted_at / created_date — same fall-through chain as Bug #46) before slicing. 4 new unit tests cover happy path, oldest-first when capped (newest-first input → 100 oldest saved), no-limit-means-unbounded (manual backfills preserved), dedup-before-cap. All 4 fetcher skills updated to v4-v5 with `--data '{"limit": 100}'`. **Real foundation fix (`bulk_save_tracked`) queued for next session** in the OS hardening sprint — single Pydantic validation pass + insert_many + batched audit chain + batched watch evaluation.

**Bug #12 REFRAME.** Per Craig 2026-05-04: "the mongo DB in the secret is needed. its just us that needs to use this other value." The shared `mongodb-uri` AWS Secret correctly stores the private-link host (`dev-indemn-pl-0`) the platform NEEDS — resolves only inside AWS VPCs. Sessions 15+16's "re-fixes" (overwriting with the public host) were the actual bug; the Railway platform's auto-restore was correct behavior, not a regression. Fix in operating-system worktree commit `3ddc02a`: `scripts/secrets-proxy/mongosh-connect.sh` does inline `-pl-0` → `` host swap (`LOCAL_URI="${URI/-pl-0/}"`); pattern catches both `dev-indemn-pl-0` and `prod-indemn-pl-0`. Comment block in script tells future operators NOT to "fix" by writing public host back to shared secret. Verified `db.runCommand({ping: 1})` returns `ok: 1`.

**7 zombie polling loops killed.** Process audit found `until [...]; do sleep 10/8; done` shells from prior sessions — `94677` (Thu01PM) hammering api.os.indemn.ai/api/_meta/queue-stats every 10s for 3 days; `90472` (Fri09AM) polling LangSmith every 8s; 5 stale `/tmp/claude-501` output-polling shells from prior background tasks. **Dev API response time recovered: 5-8s → 60-100ms** post-cleanup. The zombies were genuinely loading the API.

**TD-1 done-test verified end-to-end:** all 4 fetcher actors active with `mode=cron_runner` and correct cron schedules; ~17 cron completions in last 30 min (Email 7, Slack 8, Meeting 2); substantial entity counts (3375 emails, 305 meetings, 867 SlackMessages, 1379 documents); ReviewItem entity (9 fields) + Reviewer role wired; EC/TS suspended, IE active; Document.source enum includes `slack_file_attachment`; Voice OS Assistant + log-touchpoint v3 active; **ZERO dead_letter messages since Bug #50 deploy at 21:33 UTC**.

**Load-bearing turns:**

1. **Pre-flight Bug #49 verification surfaced two MORE bugs (#50a + #50b).** What was supposed to be a 2-minute confirmation of the heartbeat fix became a multi-hour root-cause investigation — but caught both the visibility-extend gap (Bug #49 only fixed half the problem) and the max_attempts-cap regression (recovery path bypassed `bus.fail`'s cap). Lesson: pre-flight verification is real engineering work, not a checkbox. The discipline of "actually run the done-test" prevents TD-2 from landing on broken foundations.

2. **The `fetch_new` bottleneck was diagnosed by code-reading, not by adding telemetry.** Craig pushed back on subprocess-timing as the diagnostic approach ("isn't there a better way to get this information using the CLI?"). The right answer was reading `kernel/capability/fetch_new.py:77-96` and seeing the sequential `for item in raw_results: await save_tracked(...)` loop. This is a `bulk_save_tracked` shape — proper foundation work, not a chunk-cap bandaid. The chunk-cap shipped today is the bridging fix; the proper fix is queued for the OS hardening sprint.

3. **Sessions 15 + 16 had been fighting Bug #12's auto-restore for weeks.** Per Craig's 2026-05-04 reframe: the platform NEEDS the private-link host (`dev-indemn-pl-0`), the auto-restore is correct platform behavior, our local wrapper was the wrong tool reading the wrong source-of-truth. The fix isn't a new shared secret — it's an inline host swap in the wrapper. Lesson: when something keeps "regressing" through the same fix N times, the fix is wrong. Stop fixing; reframe.

4. **CLI debugging surface is materially undersized for production work.** Session 17's investigation reached for `mongosh` + `railway logs --build/deployment` + `aws secretsmanager get-secret-value` + `time` — none of which should have been necessary if the CLI surfaced what's already in OTEL + the kernel. Logged comprehensively in os-learnings.md as "CLI debugging gap." Proposed `indemn diagnose` command group is queued for OS hardening sprint Tier 1 #2. The `indemn` CLI being the primary diagnostic surface is a vision §2 dogfood-the-OS commitment we haven't honored yet.

5. **Pre-TD-2 OS hardening sprint queued — building robust software is a discipline, not an aspiration.** Per Craig 2026-05-04: "I am in the business of building robust software. I hate bugs that impede our ability to work effectively with the OS or cause issues." Session 18 will be a focused OS-quality session before TD-2's cascade goes on top. Tier 1 list: `fetch_new` bulk_save_tracked, `indemn diagnose` command group, list endpoint arbitrary field filters regression fix, `--include-related` polymorphic Option B support, Employee `entity_resolve` activation, plus os-learnings.md status-badge audit. Prompt drafted at `projects/customer-system/PROMPT-2026-05-05-os-hardening.md`.

**Material state at close:** TD-1 verified done end-to-end. Bug #50 + fetch_new chunk-cap + Bug #12 wrapper fix all deployed. 7 zombies killed. Dev API healthy (~60-100ms). EC/TS suspended (unchanged); IE active (unchanged). 4 fetcher skills at v4-v5 with limit:100. Pricing Framework Session 3+ ran in parallel and reached Phase D visual v6 generated from full staging JSON — different worktree, Workstream A files preserved verbatim per parallel-session discipline.
