...
## Where we are now (2026-05-05)

**Pre-TD-2 OS hardening sprint COMPLETE (Session 18).** All 5 Tier 1 items shipped, deployed, verified end-to-end. 514 unit tests pass (+33 new). Foundation is clean for TD-2.

**Session 18 delivered:**
1. `bulk_save_tracked()` — batched insert replaces sequential per-entity save loop in fetch_new. Same audit + watch contracts. 12 tests.
2. `indemn diagnose` command group — actor/message/cron sub-commands. Verified live against Email Fetcher. 12 tests.
3. List endpoint `--data` filter fix — CLI was missing the flag. Bogus fields → 400. 4 tests.
4. Polymorphic `--include-related` — `is_polymorphic_relationship` + `target_type_field`. Touchpoint entity def activated. 5 tests.
5. Employee `entity_resolve` activated — verified Kyle@indemn.ai → score 1.0.
6. os-learnings.md 7 rows corrected. indemn-os CLAUDE.md updated.

**TD-2 cascade activation begins next session (Session 19).** Per `roadmap.md § TD-2`: 7-associate cascade (EC v9→v10, MC NEW, SC NEW, TS v6→v7, IE verified, Proposal-Hydrator NEW, Company-Enricher NEW) + ReviewItem universal-escape-valve. Trace-as-build-method — start with MeetingClassifier on Armadillo's Apr 28 discovery meeting.

For deeper context: `CURRENT.md`, `SESSIONS.md` Session 18 entry.
