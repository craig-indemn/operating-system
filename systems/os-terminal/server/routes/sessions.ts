...
import { readAllSessions, findById, findByName, getSessionCli, getSessionsDir } from '../state.js';
import { isValidSessionName, TMUX_BIN } from '../tmux.js';
...
const ALLOWED_TYPES = ['claude', 'shell'] as const;
...
router.post('/', asyncHandler(async (req, res) => {
  const { name, type, addDirs, model, permissions } = req.body;
  ...
  const sessionType = type || 'claude';
  if (!ALLOWED_TYPES.includes(sessionType)) {
    res.status(400).json({ error: `Invalid type. Allowed: ${ALLOWED_TYPES.join(', ')}` });
    return;
  }

  if (sessionType === 'shell') {
    // Create a plain tmux session directly — skip session CLI
    const tmuxName = `os-${name}`;
    try {
      await exec(TMUX_BIN, ['new-session', '-d', '-s', tmuxName, '-x', '200', '-y', '50'], {
        timeout: 10_000,
      });
      ...
      await writeFile(join(getSessionsDir(), `${sessionId}.json`), JSON.stringify(state, null, 2));
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  if (session.type === 'shell') {
    // Shell session: kill tmux directly and remove state file
    try {
      await exec(TMUX_BIN, ['kill-session', '-t', `os-${session.name}`], { timeout: 10_000 });
    } catch {
      // tmux session may already be gone — that's fine
    }
    try {
      await unlink(join(getSessionsDir(), `${session.session_id}.json`));
    } catch {
